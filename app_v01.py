...
def chat_respond():
    ...
    comment, question = get_conversation_response(student_text, history, topic_name)
    ...
    # Pre-generate comment audio for review
    comment_audio = make_audio_b64(comment)
    question_audio = make_audio_b64(question) if not is_last else None

    if is_last:
        closing = "Great conversation! Let me save your session and prepare your review."
...
@login_required
def chat_finish():
    """Save session to DB, run analysis, generate progress report if needed."""
    turns      = session.get("turns", [])
    topic_name = session.get("topic_name", "")
    ...
    db.session.flush()  # get db_session.id
    ...
    if session_number % 5 == 0:
        # Fetch last 5 sessions with their analyses
    ...
    # Build review data for frontend
    review_turns = []
    for t in turns:
        review_turns.append({
            "speech":        t["student_speech"],
            "comment":       t["fluency_comment"],
            "comment_audio": make_audio_b64(t["fluency_comment"]),
        })
